...
class FasterRCNN_Evaluator:
    ...
    def process_image_raw(self, img):
        out_cls_pred, out_rpn_rois, out_bbox_regr, dims = self.process_image_raw_detailed(img)
        labels = out_cls_pred.argmax(axis=1)
        regressed_rois = regress_rois(out_rpn_rois, out_bbox_regr, labels, dims)

        return regressed_rois, out_cls_pred

    def process_image_raw_detailed(self, img):
        _, cntk_img_input, dims = resize_and_pad(img, self._img_shape[2], self._img_shape[1], pad_value=114)
